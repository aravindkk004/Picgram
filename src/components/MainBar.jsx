...
const MainBar = ({currUser}) => {
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="flex md:items-start justify-center w-full">
      ...
      {isPostLoading ? (<Loader />
        ...
        <ul className="flex flex-col flex-1 gap-9 w-full md:mb-0 mb-[50%] md:p-0 px-3">
          {posts.length > 0 ? (
            posts.map((post, index) => (
              <li className="flex justify-center w-full" key={index}>
                <PostCard post={post} user={post.user} currUser={currUser}/> 
              </li>
            ))
          ) : (
